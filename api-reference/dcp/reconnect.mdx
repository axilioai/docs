...
cursor. Preserve the URL's existing query parameters:
...
1. Connect with `resume=1` and keep the existing token query parameter.
...
5. If resync is required, rebuild device state or backfill the frame archive.
